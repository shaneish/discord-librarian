...
class Collector(commands.Cog):
    ...
    async def collect(self, message, *args):
        num_messages = None
        f = pathlib.Path(__file__).parent / "resources" / f"{message.channel}-{datetime.now().strftime('%Y%m-%d%H-%M%S-')}.csv"
        if len(args) > 0:
            if args[0].isdigit():
    ...
    async def collect_all(self, ctx, *args):
        num_messages = None
        f = pathlib.Path(__file__).parent / "resources" / f"all_messages-{datetime.now().strftime('%Y%m-%d%H-%M%S-')}.csv"
        if len(args) > 0:
            if args[0].isdigit():
                num_messages = int(args[0])
        ...
        os.rename(f, pathlib.Path(__file__).parent / "resources" / "server_messages.csv")