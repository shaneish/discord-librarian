...
class Creeper(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if or_includes(message.content, "socialis", "communis") and (
            random.random() < 0.33
        ):
            # You know what this does
        ...
        if (
            or_includes(message.content, "shane", "metricity", "the best")
            and (message.author != self.bot.user)
            and (random.random() < 0.333)
        ):
            await message.channel.send(f"Shane really is the best.")

        if (
            or_includes(message.content, "suck", "sux")
            and (message.author != self.bot.user)
            and (random.random() < 0.333)
        ):
        ...
        if ("twitter" in message.content.lower()) and (random.random() < 0.25):
            if "metricity" in message.author.name.lower():
                await message.channel.send(
                    f"Another well-researched Shaun King tweet, {message.author.mention}?"
                )
...
class RateLimiter(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if isinstance(message.channel, discord.channel.DMChannel) or isinstance(
            message.channel, discord.channel.GroupChannel
        ):
            return
        if not self.last_check_in or self.last_check_in < (
            message.created_at - timedelta(seconds=60)
        ):
